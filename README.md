...
Примечание: в `DataLoader.rename_columns()` применяется нормализация заголовков (`strip`, нормализация символов, удаление пунктуации), поэтому небольшие вариации заголовков Excel (пробелы/диакритика/скобки) не ломают mapping.
